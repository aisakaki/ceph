...
public:
  ...
  get_extent_ret<T> get_extent(
    ...
    if (!cached->has_actual_data()) {
      auto ret = TCachedExtentRef<T>(static_cast<T*>(cached.get()));
      on_cache(*ret);
      SUBDEBUG(seastore_cache,
        "{} {}~{} is present without actual data, reading ... -- {}",
        T::TYPE, offset, length, *ret);
  ...
  get_extent_if_cached_ret get_extent_if_cached(
    ...
    if (!ret ||
      ...
      if (!ret) {
        ...
        SUBDEBUGT(seastore_cache, "{} {} is absent{}",
                  t, type, offset, "(placeholder)");
      } else {
        SUBDEBUGT(seastore_cache, "{} {} is present without actual data",
                t, type, offset);
      }
      return get_extent_if_cached_iertr::make_ready_future<
        CachedExtentRef>();
    ...
    // present in cache with actual data and is not a retired_placeholder
  ...
  get_extent_iertr::future<TCachedExtentRef<T>> get_extent(
	...
	return seastar::make_ready_future<TCachedExtentRef<T>>(
      ...
      if (result == Transaction::get_extent_ret::ABSENT) {
        ...
        assert(!ret->has_actual_data());
        SUBTRACET(seastore_cache, "{} {}~{} is present on t without data,\
          query cache ...", t, T::TYPE, offset, length);
      }
   ...
   *
   * find extents from
   * - t if old extent in transaction and has actual data
   * - extent_set if old extent in cache and has actual data
   * and return the bufferptr.
   * return std::nullopt if cannot find in both t and extent_set.
   */
  std::optional<ceph::bufferptr> get_remapped_buffer(
    Transaction &t,
    paddr_t paddr) {
    LOG_PREFIX(Cache::get_remapped_buffer);
    CachedExtentRef ret;
    auto result = t.get_extent(paddr, &ret);
    if (result == Transaction::get_extent_ret::PRESENT
      && ret->has_actual_data()) {
      SUBTRACET(seastore_cache, "get buffer from t, length: {}",
        t, ret->get_length());
      return ret->get_bptr();
    } else if (auto cached = query_cache(paddr, nullptr)) {
      SUBTRACET(seastore_cache, "get buffer from cache, length: {}",
        t, cached->get_length());
      return cached->get_bptr();
    } else {
      SUBTRACET(seastore_cache, "no raw data in memory, get std::nullopt", t);
      return std::nullopt;
    }
  }
