...
struct extent_to_write_t {
  ...
  static extent_to_write_t create_existing(
    LBAPinRef &&pin, laddr_t addr, extent_len_t len) {
...
private:
  ...
  extent_to_write_t(LBAPinRef &&pin, laddr_t addr, extent_len_t len)
...
struct extent_to_remap_t {
  ...
  static extent_to_remap_t create_remap(
    LBAPinRef &&pin, extent_len_t new_offset, extent_len_t new_len) {
...
private:
  extent_to_remap_t(
    LBAPinRef &&pin, extent_len_t new_offset, extent_len_t new_len)
    : type(type_t::REMAP), pin(std::move(pin)), new_offset(new_offset), new_len(new_len) {}

  extent_to_remap_t(
    LBAPinRef &&pin, extent_len_t new_offset,
...
using extent_to_remove_list_t = std::list<LBAPinRef>;
...
overwrite_ops_t prepare_ops_list(
  ...
  // overwrite happen in one original extent.
  ...
  //prepare to_remap, overwrite happen in multiple extents
  if (front.is_existing()) {
	    ...
	    to_write.push_back(extent_to_write_t::create_zero(
	      ...
	      object_data.get_reserved_data_len() -
            ...
            return read_pin(
              ctx,
              pin.duplicate()
            ).si_then([ctx, size, pin_offset, append_len, &pin, &pins,
