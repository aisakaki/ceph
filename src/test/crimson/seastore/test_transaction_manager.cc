...
struct transaction_manager_test_t :
  ...
  TestBlockRef try_alloc_extent_by_pin(
    test_transaction_t &t,
    LBAPinRef &&pin) {
    using ertr = with_trans_ertr<TransactionManager::read_extent_iertr>;
    using ret = ertr::future<TestBlockRef>;
    ...
    test_mappings.alloced(pin->get_key(), *ext, t.mapping_delta);
    return ext;
  }
  ...
  LBAPinRef try_get_pin(
    test_transaction_t &t,
    laddr_t offset) {
    ceph_assert(test_mappings.contains(offset, t.mapping_delta));
    using ertr = with_trans_ertr<TransactionManager::get_pin_iertr>;
    using ret = ertr::future<LBAPinRef>;
    auto pin = with_trans_intr(*(t.t), [&](auto& trans) {
      return tm->get_pin(trans, offset);
    }).safe_then([](auto pin) -> ret {
      return ertr::make_ready_future<LBAPinRef>(std::move(pin));
    }).handle_error(
      [](const crimson::ct_error::eagain &e) {
	return seastar::make_ready_future<LBAPinRef>();
      },
      crimson::ct_error::assert_all{
	"get_extent got invalid error"
      }
  ...
  LBAPinRef remap_pin(
    ...
    auto pin = with_trans_intr(*(t.t), [&](auto& trans) {
      return tm->remap_pin<TestBlock>(trans, std::move(opin), new_offset, new_len);
    ...
    auto extent = try_alloc_extent_by_pin(t, pin->duplicate());
    if (extent) {
  ...
  using overwrite_pin_ret = std::tuple<LBAPinRef, TestBlockRef, LBAPinRef>;
  overwrite_pin_ret overwrite_pin(
    ...
    ceph::bufferlist &bl) {
    if (t.t->is_conflicted()) {
      return std::make_tuple<LBAPinRef, TestBlockRef, LBAPinRef>(nullptr, nullptr, nullptr);
    }
    auto o_laddr = opin->get_key();
    auto o_paddr = opin->get_val();
    auto o_len = opin->get_length();
    auto res = with_trans_intr(*(t.t), [&](auto& trans) {
      return tm->overwrite_pin<TestBlock>(
        trans, std::move(opin), new_offset, new_len, bl);
    }).handle_error(crimson::ct_error::eagain::handle([] {
      return std::make_tuple<LBAPinRef, TestBlockRef, LBAPinRef>(
    ...
    if (lpin) {
      ...
      auto lext = try_alloc_extent_by_pin(t, lpin->duplicate());
      if (lext) {
    ...
    if (rpin) {
      ...
      auto rext = try_alloc_extent_by_pin(t, rpin->duplicate());
      if (rext) {
	  ...
	  return seastar::async([&] {
		...
		return;
	      ...
	      if (get_random_contents() % 2 == 0) {
                auto ext = try_get_extent(t, pin->get_key());
                if (ext) {
	...
	[&](auto) {
	  return seastar::async([&] {
		...
		return;
              ...
              // lpin is nullptr might not cause by confliction, it might just not exist.
              if (lpin) {
		  ...
		  ASSERT_TRUE(lext1->is_exist_mutation_pending());
              ...
              ASSERT_TRUE(rpin);
              auto rext = try_get_extent(t, rpin->get_key());
              if (!rext) {
		conflicted++;
		return;
              }
	      if (get_random_contents() % 2 == 0) {
                auto rext1 = mutate_extent(t, rext);
                ASSERT_TRUE(rext1->is_exist_mutation_pending());
	      }
